...
Познакомиться с методами отладки при помощи GDB и его возможностями.

# Задание

Написать неоюходимые программы и изучить методы откладки с помощью GDB.

# Выполнение лабораторной работы

## Реализация подпрограмм в NASM

Создадим каталог для программам лабораторной работы № 9, перейдем в него и со-
здадим файл lab9-1.asm. ([рис. @fig-001]).
...
Снова открываем файл для редактирования и изменяем его, добавив добавив подпрограмму. ([рис. @fig-004]).
...
Создадим файл lab9-2.asm в каталоге ~/work/arch-pc/lab09. ([рис. @fig-006]).

![Создаем файл помощью команды touch](image/6.png){#fig-006 width=70%}
...
1.Порядок операндов: В синтаксисе AT&T порядок операндов обратный - сначала указывается исходный операнд, затем результирующий. В синтаксисе Intel порядок прямой - сначала результирующий операнд, затем исходный.

2.Разделители: В синтаксисе AT&T операнды разделяются запятыми. В синтаксисе Intel в качестве разделителей могут использоваться как запятые, так и косые черты.

3.Префиксы размера операндов: В синтаксисе AT&T размер операнда указывается перед операндом с помощью префиксов: "b" для байта, "w" для слова, "l" для двойного слова. В синтаксисе Intel размер указывается после операнда с помощью суффиксов: "b", "w", "d".

4.Обозначение непосредственных значений: В синтаксисе AT&T непосредственные значения предваряются символом "$". В синтаксисе Intel непосредственные значения могут указываться без специального символа.

5.Обозначение адресов памяти: В синтаксисе AT&T адреса памяти заключаются в круглые скобки. В синтаксисе Intel адреса указываются без скобок.

6.Обозначение регистров: В синтаксисе AT&T имена регистров начинаются с символа "%". В синтаксисе Intel регистры обозначаются без этого символа, например "EAX" вместо "%eax".
...
Копируем файл lab8-4.asm в файл с именем lab09-4.asm. ([рис. @fig-028]).
...
![Проверяем работу программы, она работает неправильно](image/33.png){#fig-033 width=70%}
...
![Создаем и запускаем файл, он работает корректно](image/36.png){#fig-036 width=70%}
